...
figure
plot( data.time, data.motor_pos, 'o' ); hold on
data.motor_pos = data.motor_pos/volt_to_deg_potentiometer/deg_to_rad-5;
plot( data.time, data.motor_pos , 'o' );

minv = min(data.motor_pos);
maxv = max(data.motor_pos);

new_vtd = (maxv-minv)/352;

data.motor_pos = (data.motor_pos - minv) / new_vtd * deg_to_rad;
plot( data.time, data.motor_pos , 'o' );

motor_pos_int = int_potentiometer( data.motor_pos, data.time );
figure
plot( data.time, motor_pos_int , 'o' );
...
for idx = 2:length( motor_pos )
    ...
    if ( motor_pos(idx) ) > 6.13 && motor_pos(idx-1) <= 6.13 && trasla == false
        trasla = true;
        risalita = 0;
        last_pos = idx-1;
    ...
    if trasla == true       % il giro è finito
        ...
        if motor_pos(idx) > 0.002 || risalita < 5
